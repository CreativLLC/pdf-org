...
# Google Drive MCP entry is included but disabled by default per ADR-0015.
# To enable: uncomment the "google-drive" block, populate
# GOOGLE_DRIVE_API_KEY in ~/.archon/credentials/<alias>/.envrc, and
# restart Claude Code.
cat > "$ENGAGEMENT_DIR/.mcp.json" <<EOF
{
  ...
  "mcpServers": {
    ...
    "_google_drive_disabled_until_engaged": {
      "_note": "Per ADR-0015. Rename the key to 'google-drive' (drop the underscore prefix) and provide GOOGLE_DRIVE_API_KEY via direnv to enable. Used by /sf to read Google Drive docs linked in Jira ticket ## Context sections.",
      "command": "uvx",
...
EOF
ok "Wrote .mcp.json (Google Drive MCP entry disabled by default; see ADR-0015)"
...
# security/ — per ADR-0013. Has its own internal structure:
# README + 4-6 top-level files + profiles/ + permission-sets/ subdirs.
# /sf-discover-org populates these. Bootstrap just creates the scaffold
# so the directories exist (otherwise awesome-pages doesn't render them
# in the sidebar before first discovery).
mkdir -p "$ENGAGEMENT_DIR/docs/security/profiles"
mkdir -p "$ENGAGEMENT_DIR/docs/security/permission-sets"

cat > "$ENGAGEMENT_DIR/docs/security/README.md" <<README_EOF
---
title: Security
audience: public
last_updated: $TODAY
...
# Security — $CLIENT_NAME

Consolidated security and access-control posture of the Salesforce org. Per [ADR-0013](https://github.com/CreativLLC/archon-salesforce-jira/blob/main/decisions/0013-engagement-security-documentation.md).

## What's in this section

This category is populated by \`/sf-discover-org\`'s three security nodes. After the first discover run, this README is regenerated and lists every file. Before that, the section is empty.

Expected contents once populated:

- \`sharing-model.md\` — Org-Wide Defaults, sharing rules, Apex sharing, implicit sharing, Restriction Rules.
- \`custom-permissions.md\` — Custom Permissions + where they're checked in Apex / Flow.
- \`public-groups-and-queues.md\` — Public Groups + Queues.
- \`apex-sharing.md\` — Programmatic sharing in Apex code.
- \`permission-set-groups.md\` — *Only if any PSGs exist in this engagement.*
- \`profiles/\` — One file per profile.
- \`permission-sets/\` — One file per permission set.

*Empty until \`/sf-discover\` populates this section.*
README_EOF

cat > "$ENGAGEMENT_DIR/docs/security/profiles/README.md" <<README_EOF
---
title: Profiles
audience: public
last_updated: $TODAY
...
One file per profile in \`force-app/main/default/profiles/\`. Auto-generated by \`/sf-discover-org\`'s \`document-security-profiles\` step.

## Profile inventory

*Empty until \`/sf-discover\` populates this section.*

If this engagement uses only stock Salesforce profiles (and they haven't been retrieved into the SFDX project), this README will say so after the first discover run.
README_EOF

cat > "$ENGAGEMENT_DIR/docs/security/permission-sets/README.md" <<README_EOF
---
...
# Permission Sets — $CLIENT_NAME

One file per permission set in \`force-app/main/default/permissionsets/\`. Auto-generated by \`/sf-discover-org\`'s \`document-security-permission-sets\` step.
...
The 'why' of $CLIENT_NAME's Salesforce org: system overview, subsystem boundaries, integration topology, and any cross-cutting design notes that span multiple objects or processes. These docs are **engineer-authored synthesis** — they explain WHY the system is shaped the way it is. Auto-generated reference content lives in [\`../objects/\`](../objects/) (per-object detail), [\`../flows/\`](../flows/) (per-flow), [\`../integrations/\`](../integrations/) (external systems), and [\`../security/\`](../security/) (sharing + access).

## Index

*Empty until engineers author content. None of the files below are auto-generated. Add them when they earn their place.*
...
- Other cross-cutting design docs as they're needed.

> The **sharing / security model** lives in [\`../security/\`](../security/), not here. Per [ADR-0013](https://github.com/CreativLLC/archon-salesforce-jira/blob/main/decisions/0013-engagement-security-documentation.md). \`/sf-discover-org\` auto-generates a consolidated [\`../security/sharing-model.md\`](../security/sharing-model.md) plus per-profile and per-permission-set docs.

Architecture docs have no canon template — pattern after [ADR-0010](https://github.com/CreativLLC/archon-salesforce-jira/blob/main/decisions/0010-engagement-documentation-model.md) and other engagement docs in the harness exemplar.
README_EOF
